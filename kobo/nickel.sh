...
PLATFORM=freescale                                                              
if [ `dd if=/dev/mmcblk0 bs=512 skip=1024 count=1 | grep -c "HW CONFIG"` == 1 ];
...
( /usr/local/Kobo/pickel disable.rtc.alarm                                      
  if [ ! -e /etc/wpa_supplicant/wpa_supplicant.conf ]; then                     
    cp /etc/wpa_supplicant/wpa_supplicant.conf.template /etc/wpa_supplicant/wpa_
  fi                                                                            
...
if [ ! -e /usr/local/Kobo/platforms/libkobo.so ]; then                          
    ...
    [ `cat /mnt/onboard/.kobo/Kobo/Kobo\ eReader.conf | grep -c dhcpcd=true` == 
