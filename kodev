...
function kodev-release() {
    HELP="
USAGE: $0 ${CMD} <OPTIONS> <TARGET> [TYPES]…
...
"
    parse_options "i${BUILD_GETOPT_SHORT}" "ignore-translation,${BUILD_GETOPT_LONG}" '+' "$@"
    ...
    shift
    [[ $# -gt 0 ]] || set -- update
    run_make ${NO_BUILD:+--assume-old=all} "$@"
}
