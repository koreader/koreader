    ...
    if [ "${kpv_launch_count}" = "" ] || [ "${kpv_launch_count}" = "0" ]; then
        ...
        if [ "$(version "${FW_VERSION}")" -ge "$(version "5.6.5")" ]; then
            ...
            if [ "$(version "${FW_VERSION}")" -ge "$(version "5.7.2")" ]; then
                if [ "${FROM_KUAL}" = "yes" ]; then
                    # NOTE: Simply setting com.lab126.winmgr liglPause would be enough, but the stock system toggles this flag on its own during suspend/resume,
                    #       so we can't reliably take it over...
