...
for i in $(ls l10n); do
    if [ "${i}" != "templates" ]; then
        strings_file="l10n/${i}/mac.strings"
        if [ -f "${strings_file}" ]; then
            echo "bundling translation for $i"
            mkdir -p "${APP_PATH}/Contents/Resources/${i}.lproj"
            cp -p "${strings_file}" "${APP_PATH}/Contents/Resources/${i}.lproj/MainMenu.strings"
        fi
    fi
done
