...
if [ ${RETURN_VALUE} -ne ${KO_RC_HALT} ]; then
    ...
    if [ "${VIA_NICKEL}" = "false" ]; then
        if pkill -0 kbmenu; then
            # If we were started by KSM and requested an exit, do *NOT* exit the script so as not to re-enter KSM at all,
            # to make sure the device powers off with our own ScreenSaver displayed.
            while :; do
                sleep 2073600
            done
        fi
