...
Generated by tools/gen_lua_rules.py in https://github.com/iDynbek/kazsearch-py
from kazsearch/rules.py — do not edit by hand. Regenerating is what keeps this
table and the Python reference from drifting apart.

Kazakh suffixes attach in a fixed order, so each position is a "layer" and the
stemmer walks them left to right. `n` is the suffix length in CHARACTERS (not
bytes), `h` is the vowel-harmony class it demands, `w` marks suffixes that are
ambiguous with ordinary root material.

Derived from pg-kazsearch by darkhanakh
(https://github.com/darkhanakh/pg-kazsearch), licensed LGPL-3.0-or-later, and
distributed here under KOReader's AGPL-3.0-or-later.
--]]
