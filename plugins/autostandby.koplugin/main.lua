...
function AutoStandby:init()
    if not self.settings:has('filter') then
        ...
        self.settings
        -- If forbidden, standby is never allowed to occur
        :saveSetting('forbidden', false)
        -- Consider input only further than this many seconds apart
        :saveSetting('filter', 1)
        -- Initial delay period during which we won't standby
        :saveSetting('min', 1)
        -- Multiply the delay with each subsequent input that happens, scales up to max
        :saveSetting('mul', 1.5)
        -- The multiplier is applied only if event arrives in (current_delay + win) period.
        :saveSetting('win', 5)
        -- Input that happens further than 30 seconds since last input one reset delay back to 'min'
        :saveSetting('max', 30)
        -- If battery is below this percent, make auto-standby aggressive again (disables scaling by mul)
        :saveSetting('bat', 60)
        :flush()
    end
...
function AutoStandby:addToMainMenu(menu_items)
    menu_items.autostandby = {
        ...
        sub_item_table = {
            ...
            self:_config_item(_("Min input idle seconds"), "min", function() return 0 end, function() return self.settings:readSetting("max") end),
            self:_config_item(_("Max input idle seconds"), "max", function() return 0 end),
            self:_config_item(_("Input window seconds"), "win", function() return 0 end, function() return self.settings:readSetting("max") end),
            self:_config_item(_("Always standby if battery below"), "bat", function() return 0 end, function() return 100 end),
        }
    }
end
...
function AutoStandby:_config_item(text, cfg, min, max)
    return {
        keep_menu_open = true,
        text = text,
