...
function Japanese:init()
    ...
    self.scan_length = G_reader_settings:readSetting("jpn_text_scan_length") or DEFAULT_TEXT_SCAN_LENGTH
...
function Japanese:onWordSelection(args)
    ...
    until current_end == nil or num_expansions >= self.scan_length
        ...
        sub_item_table = {
            ...
            },
            {
                ...
                callback = function()
                    ...
                    for _, rules in pairs(self.deinflector.rules) do
                        nvariants = nvariants + #rules
                        nrules = nrules + 1
                    end
                    UIManager:show(InfoMessage:new{
                        text = T(_("Deinflector has %1 rules and %2 variants loaded."), nrules, nvariants),
                    })
                end,
            },
            -- Allow configuring self.scan_length.
            {
                text_func = function()
                    return T(_("Text scan length: %1"), self.scan_length)
                end,
                keep_menu_open = true,
                callback = function()
...
Larger values allow longer phrases to be selected automatically, but with the tradeoff that selections may become slower.

Default value: 20]]),
                        width = math.floor(Screen:getWidth() * 0.75),
                        value = self.scan_length,
                        value_min = 5,
                        value_max = 1000,
                        value_step = 1,
                        value_hold_step = 5,
                        ...
                        callback = function(spin)
                            self.scan_length = spin.value
                            G_reader_settings:saveSetting("jpn_text_scan_length", self.scan_length)
                        end
                    }
