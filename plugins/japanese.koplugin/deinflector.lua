    ...
    for _, conversion in pairs(ALL_TEXT_CONVERSIONS) do
        ...
        table.insert(item_table, {
            ...
            callback = function(touchmenu_instance)
                ...
                G_reader_settings:saveSetting("japanese_text_conversions", self.enabled_text_conversions)
    ...
    return {
        {
            ...
            callback = function()
                ...
                UIManager:show(InfoMessage:new{
                    -- @translators %1 is the "%1 rule(s)" string, %2 is the "%2 variant(s)" string.
        ...
        },
        {
            text_func = function()
                ...
                local nenabled_str = _("none enabled")
                if nenabled ~= 0 then
                    nenabled_str = T(N_("%1 enabled", "%1 enabled", nenabled), nenabled)
                end
                -- @translators %1 is the "%1 enabled" string.
                return T(_("Text conversion methods (%1)"), nenabled_str)
            end,
...
function Deinflector:init()
    self.enabled_text_conversions = self.enabled_text_conversions or
                                    G_reader_settings:readSetting("japanese_text_conversions") or
