...
-- before the base kana because half-width katakana use two characters for
-- 濁点・半濁点 and thus need to be replaced first. We could make use of NFD
-- normalisation, only replace the base kana (leaving the combining ◌゙・◌゚ marks
-- intact), then re-normalise with NFC but it's simpler to do it this way.
...
for i, c in ipairs(HALFWIDTH_KATAKANA) do HALFWIDTH_KATAKANA[i] = Utf8Proc.normalise(c) end
for i, c in ipairs(FULLWIDTH_KATAKANA) do FULLWIDTH_KATAKANA[i] = Utf8Proc.normalise(c) end
for i, c in ipairs(FULLWIDTH_HIRAGANA) do FULLWIDTH_HIRAGANA[i] = Utf8Proc.normalise(c) end
...
-- and utf8proc_map_custom are a bit unwieldy to use for character conversion),
-- so we have to use gsub to do all conversions.
    ...
    for mapfn_bitmask = 0, max_mapfn_bitmask do
        ...
        if not seen[mapped_text] then
            if text ~= mapped_text then
                logger.dbg("japanese.koplugin deinflector: trying mapped variant", text, "->", mapped_text)
            end
