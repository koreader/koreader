...
function Gestures:renamePresetAction(action_key_to_modify, old_preset_value, new_preset_value)
...
function Gestures:deletePresetAction(action_key_to_delete, preset_value_to_delete)
    local changed_anything = false
    for _, section_name in ipairs({ "gesture_fm", "gesture_reader" }) do
        local section_data = self.settings_data.data[section_name]
        if section_data then
            for gesture_name, actions in pairs(section_data) do
                -- Check if the gesture's actions table exists and contains the target action key
                -- with the preset value to delete.
                if type(actions) == "table" and actions[action_key_to_delete] and actions[action_key_to_delete] == preset_value_to_delete then
                    ...
                    -- check if the entire 'actions' table for this gesture is now effectively empty
                    ...
                    if not has_other_actual_actions then
                        section_data[gesture_name] = nil -- remove the entire gesture's action table if empty
                    end
                    self.updated = true
                    changed_anything = true
                end
