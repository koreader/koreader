...
function GeminiPlugin:init()
    ...
    if self.ui and self.ui.link then
        if self.ui.link.registerScheme then
            for _,scheme in ipairs(self:supportedSchemes()) do
        ...
        self.ui.link:addToExternalLinkDialog("23_gemini", function(this, link_url)
            return {
                ...
                show_in_dialog_func = function(u)
                    ...
                    return util.arrayContains(self:supportedSchemes(), scheme)
                end,
            }
        end)
    end
...
function GeminiPlugin:saveBody(body, mimetype, purl)
...
function GeminiPlugin:openBody(body, mimetype, purl, cert_info, replace_history)
    ...
    if not DocumentRegistry:hasProvider(tn) then
        UIManager:show(ConfirmBox:new{
            text = T(_("Can't view file (%1). Save it instead?"), mimetype),
            ok_text = _("Save file"),
...
function GeminiPlugin:openCurrent()
...
function GeminiPlugin:writeDefaultBookmarks()
...
function GeminiPlugin:openUrl(article_url, opts)
    ...
    local after_err_cb = opts.after_err_cb or function() end
    if self:purl() then
    ...
    if purl and purl.scheme == "about" then
        ...
        if body then
            ...
            UIManager:show(InfoMessage:new{text = T(_("Unknown \"about:\" URL: %1"), article_url)})
        end
        return
    end

    if not purl or not purl.host then
        UIManager:show(InfoMessage:new{text = T(_("Invalid URL: %1"), article_url)})
        return
    end

    local proxy = self:getSchemeProxy(purl.scheme)
    if purl.scheme ~= "gemini" and not proxy then
        UIManager:show(InfoMessage:new{text = T(_("No proxy configured for scheme: %1"), purl.scheme)})
        return
    end

    if NetworkMgr:willRerunWhenConnected(function() self:openUrl(article_url, opts) end) then
        -- Not connected yet, nothing more to do here, NetworkMgr will forward the callback and run it once connected!
        return
    end

    local id, __, id_path = Identities:get(article_url)
    local function success_cb(f, p, mimetype, params, cert_info)
        if opts.repl_purl then
            self.repl_purl = opts.repl_purl
        end
        body_cb(f, mimetype, p, cert_info)
    end
    local function error_cb(msg, p, major, minor, server_msg)
        if major then
            msg = T(_("Server reports %1: %2"), msg, server_msg)
        end
        if major == "1" then
            self:promptInput(p, server_msg, minor == "1")
        elseif major == "6" then
                UIManager:show(ConfirmBox:new{
                    text = msg,
                    ...
                    cancel_text = _("Cancel"),
                    ok_callback = function()
                        Identities:confAt(gemini.showUrl(p), function(new_id)
                            if new_id then
                                self:openUrl(p, opts)
                            end
                        end)
...
-- Prompt user for input. May modify `purl.query`.
function GeminiPlugin:promptInput(purl, prompt, is_secret, repl, initial)
    local input_dialog
    local repl_button
    local multiline_button
    local function submit()
        local input = input_dialog:getInputText()
        purl.query = url.escape(input)
        if #url.build(purl) > 1024 then
            ...
            self:openUrl(purl, {
                repl_purl = repl_button.checked and purl,
                after_err_cb = function() self:promptInput(purl, prompt, is_secret, repl, input) end,
            })
        end
    end
    local hi = 0
    ...
    local function to_hist(i)
        ...
        hi = i
        input_dialog:setInputText(hi > 0 and self.input_history[hi] or latest_input)
        update_buttons()
    end
    input_dialog = InputDialog:new{
        title = prompt,
...
function GeminiPlugin:userPromptInput(purl)
    self:promptInput(purl, "[Input]", false, false, url.unescape(purl.query or ""))
end

function GeminiPlugin:promptUnexpectedCert(host, new_fp, old_trusted_times, old_expiry, cb)
...
function GeminiPlugin:goBack(n)
    n = n or 1
...
function GeminiPlugin:clearHistory()
...
function GeminiPlugin:onTap(_, ges)
    if self.active then
        return self:followGesLink(ges)
    end
end

function GeminiPlugin:onDoubleTap(_, ges)
    if self.active then
        return self:followGesLink(ges, true)
    end
end

function GeminiPlugin:onHoldPan(_, ges)
    self.hold_pan = true
end

function GeminiPlugin:onHoldRelease(_, ges)
...
function GeminiPlugin:onSwipe(_, ges)
    if self.active then
...
function GeminiPlugin:onFollowGeminiLink(u)
...
function GeminiPlugin:queueLinksInSelected(selected)
...
function GeminiPlugin:queueBody(body, u, mimetype, cert_info, existing_item, prepend)
...
function GeminiPlugin:queueCachedHistoryItem(h, prepend)
...
function GeminiPlugin:fetchLink(u, item, prepend)
...
function GeminiPlugin:fetchQueue()
...
function GeminiPlugin:queueLink(u, prepend)
...
function GeminiPlugin:queueItem(item, prepend)
...
function GeminiPlugin:openQueueItem(n)
    n = n or 1
...
function GeminiPlugin:popQueue(n)
    n = n or 1
...
function GeminiPlugin:clearQueue()
...
function GeminiPlugin:getSavePath(purl, mimetype, cb)
    local basename = ""
    local add_ext = false
    if purl.path then
...
function GeminiPlugin:saveCurrent()
...
function GeminiPlugin:addMark(u, desc)
    if url.parse(u) then
        self:writeDefaultBookmarks()
        local line = "=> " .. u
...
function GeminiPlugin:addMarkInteractive(uri)
    local widget
...
function GeminiPlugin:showHistoryMenu(cb)
...
function GeminiPlugin:getSchemeProxy(scheme)
    local proxy = self.scheme_proxies[scheme]
    if proxy and proxy.host then
        return proxy
    end
    if scheme == "http" or scheme == "https" then
        return self:getSchemeProxy("http(s)")
    end
end

function GeminiPlugin:supportedSchemes()
    local schemes = { "gemini", "about" }
...
function GeminiPlugin:schemeProxyMenu()
    local menu
    local kv_pairs = {}
    for scheme,proxy in pairs(self.scheme_proxies) do
...
function GeminiPlugin:viewCurrentAsText()
...
function GeminiPlugin:showCurrentInfo()
    local h = self.history[1]
    local kv_pairs = {
...
function GeminiPlugin:editQueue()
    local menu
    local items = {}
...
function GeminiPlugin:showNav(uri, showKbd)
...
function GeminiPlugin:onBrowseGemini()
    if self.active then
        self:showNav()
...
function GeminiPlugin:onGeminiBack()
    if self.active then
        self:goBack()
        return true
    end
end
function GeminiPlugin:onGeminiUnback()
    if self.active then
        self:goBack(-1)
        return true
    end
end
function GeminiPlugin:onGeminiHistory()
    if self.active then
        self:showHistoryMenu()
        return true
    end
end
function GeminiPlugin:onGeminiBookmarks()
    self:openUrl("about:bookmarks")
    return true
end
function GeminiPlugin:onGeminiMark()
    if self.active then
        self:addMarkInteractive(gemini.showUrl(self:purl()))
        return true
    end
end
function GeminiPlugin:onGeminiNext()
    self:openQueueItem()
    return true
end
function GeminiPlugin:onGeminiAdd()
    if self.active then
        self:queueCachedHistoryItem(self.history[1])
        return true
    end
end
function GeminiPlugin:onGeminiInput()
    if self.active then
        self:userPromptInput(self.history[1].purl)
        return true
    end
end
function GeminiPlugin:onGeminiReload()
    if self.active then
        self:openUrl(self.history[1].purl, { replace_history = true })
        return true
    end
end
function GeminiPlugin:onGeminiUp()
    if self.active then
...
function GeminiPlugin:onGeminiGoNew()
    self:showNav("")
    return true
end
function GeminiPlugin:onGeminiNav()
    self:showNav()
    return true
end

function GeminiPlugin:addToMainMenu(menu_items)
    menu_items.gemini = {
        sorting_hint = "search",
