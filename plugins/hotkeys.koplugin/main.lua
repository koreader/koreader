...
for i = 0, 15 do
    ...
    hotkeys_list["joy_button_" .. id] = _("Gamepad " .. name)
end
for i = 0, 5 do
    local id = Gamepad.axis_ids[i] or tostring(i)
    local name = Gamepad.axis_names[i] or ("Axis " .. i)
    hotkeys_list["joy_axis_" .. id .. "_minus"] = _("Gamepad " .. name .. " -")
    hotkeys_list["joy_axis_" .. id .. "_plus"] = _("Gamepad " .. name .. " +")
end
...
function HotKeys:addToMainMenu(menu_items)
    ...
    if Device:isSDL() then --- @todo Something like Device:hasGamepad() or :supportsGamepad()?
        ...
        for i = 0, 15 do
            local id = Gamepad.button_ids[i] or tostring(i)
            table.insert(gamepad_keys, "joy_button_" .. id)
        end
        for i = 0, 5 do
            local id = Gamepad.axis_ids[i] or tostring(i)
            table.insert(gamepad_keys, "joy_axis_" .. id .. "_minus")
            table.insert(gamepad_keys, "joy_axis_" .. id .. "_plus")
        end
        table.insert(menu_items.hotkeys.sub_item_table, {
            text = _("Gamepad"),
