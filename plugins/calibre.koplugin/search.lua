...
function CalibreSearch:find(option)
    ...
    logger.info(string.format("search done in %.3f milliseconds (%s, %s, %s, %s, %s)",
        time.to_ms(time.get_duration(start_time)),
...
function CalibreSearch:getMetadata()
    ...
    if self.cache_metadata then
        ...
        if not cache then
            ...
            if is_newer then
                logger.info(string.format(template, #cache, "cache", time.to_ms(time.get_duration(start_time))))
                return cache
            else
    ...
    logger.info(string.format(template, #books, "calibre", time.to_ms(time.get_duration(start_time))))
    return books
end
