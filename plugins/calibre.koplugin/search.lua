...
function CalibreSearch:getMetadata()
    ...
    if self.cache_metadata then
        local function cacheIsNewer(timestamp)
            ...
            -- First, we'll need the current DST status...
            local tm = os.date("*t")
            local date = os.time({year = Y, month = M, day = D, hour = h, min = m, sec = s, isdst = tm.isdst})
            -- Then, use os.date and its magic bang prefix to get an actual UTC Epoch, in order to compute the local-to-UTC offset...
