...
function CalibreWireless:onReceiveJSON(data)
    ...
    while self.buffer ~= nil do
        ...
        if ok and json then
            ...
            logger.warn("failed to decode json data", json_data)
        end
    end
end
...
function CalibreWireless:setCalibreInfo(arg)
    ...
    CalibreMetadata:setDeviceInfo(arg)
...
function CalibreWireless:getBookCount(arg)
    ...
    for index, _ in ipairs(CalibreMetadata.books) do
        ...
        -- do not send metadata that can be huge
...
function CalibreWireless:noop(arg)
    ...
    if arg.count then return end
    -- calibre requests more metadata for a book by its index
    if arg.priKey then
        local book = CalibreMetadata.books[arg.priKey]
        book.cover = nil
        book.comments = nil
        book.thumbnail = nil
        logger.dbg("Sending extra metadata for book", book.title)
        self:sendJsonData('OK', book)
        return
    end
    -- keep-alive NOOP
...
function CalibreWireless:sendToCalibre(arg)
    ...
    if self:isCalibreAtLeast(4,18,0) then
        self:sendJsonData('ERROR', {message = _("Uploading books is not supported yet")})
    else
