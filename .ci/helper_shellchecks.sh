...
SHELLSCRIPT_ERROR=0
SHFMT_OPTIONS="--indent 4 --case-indent"
...
for shellscript in "${shellscript_locations[@]}"; do
    ...
    if ! shfmt ${SHFMT_OPTIONS} --keep-padding "${shellscript}" >/dev/null 2>&1; then
        echo -e "${ANSI_RED}Warning: ${shellscript} contains the following problem:"
        # shellcheck disable=2086
        shfmt ${SHFMT_OPTIONS} --keep-padding "${shellscript}" || SHELLSCRIPT_ERROR=1
        continue
    fi
    # shellcheck disable=2086
    if [ "$(cat "${shellscript}")" != "$(shfmt ${SHFMT_OPTIONS} "${shellscript}")" ]; then
        ...
        # shellcheck disable=2086
        shfmt ${SHFMT_OPTIONS} --diff "${shellscript}" || SHELLSCRIPT_ERROR=1
    fi
done
