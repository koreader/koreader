...
for shellscript in "${shellscript_locations[@]}"; do
    ...
    if ! shfmt -i 4 -ci -kp "${shellscript}" >/dev/null 2>&1; then
        echo -e "${ANSI_RED}Warning: ${shellscript} contains the following problem:"
        shfmt -i 4 -ci -kp "${shellscript}" || SHELLSCRIPT_ERROR=1
        continue
    fi
    if [ "$(cat "${shellscript}")" != "$(shfmt -i 4 -ci -kp "${shellscript}")" ]; then
        echo -e "${ANSI_RED}Warning: ${shellscript} does not abide by coding style, diff for expected style:"
        shfmt -i 4 -ci -kp -d "${shellscript}" || SHELLSCRIPT_ERROR=1
    fi
done
