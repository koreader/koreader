...
for shellscript in "${shellscript_locations[@]}"; do
    ...
    if ! shfmt -i 4 -ci -kp -p "${shellscript}" >/dev/null 2>&1; then
        echo -e "${ANSI_RED}Warning: ${shellscript} contains the following problem:"
        shfmt -i 4 -ci -kp -p "${shellscript}" || SHELLSCRIPT_ERROR=1
        continue
    fi
    if [ "$(cat "${shellscript}")" != "$(shfmt -i 4 -ci "${shellscript}")" ]; then
        ...
        shfmt -i 4 -ci -kp -p -d "${shellscript}" || SHELLSCRIPT_ERROR=1
    fi
done
