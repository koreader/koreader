...
local Cache = {
    -- Cache configuration:
    -- Max storage space, in bytes (or number of slots if unlimited_storage is true)
    size = 8 * 1024 * 1024,
    -- Average item size is used to compute the amount of slots in the LRU.
    avg_itemsize = 8196,
    -- Should LRU not impose a storage space limitation, and only a slot count (in which case, size is taken to mean slots)?
    -- c.f., CatalogCache
    unlimited_storage = false,
    -- Should LRU call the object's onFree method on eviction? Implies using CacheItem instead of plain tables/objects.
    -- c.f., DocCache
    with_eviction_cb = false,
...
function Cache:init()
    if self.unlimited_storage then
        -- Caller doesn't care about storage space, just slot count
        self.slots = self.size
        self.cache = lru.new(self.slots, nil, self.with_eviction_cb)
    else
        ...
        self.cache = lru.new(self.slots, self.size, self.with_eviction_cb)
    end
    ...
    if not self.with_eviction_cb then
