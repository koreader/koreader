...
function PluginCompatibility.checkCompatibility(plugin_meta)
    ...
    if min_version then
        ...
        if min_ver and current_version < min_ver then
            local message = string.format("Requires KOReader %s or later (current: %s)",
                min_version, Version:getShortVersion())
    ...
    if max_version then
        ...
        if max_ver and current_version > max_ver then
            local message = string.format("Not compatible with KOReader %s and newer. Requires KOReader %s or older",
                Version:getShortVersion(), max_version)
    ...
    main_menu = Menu:new {
        ...
        onMenuChoice = function(menu_self, item)
            if item.plugin_ref then
                PluginCompatibility.showPluginOverrideMenu(
                    item.plugin_ref,
                    G_reader_settings,
                    function()
