...
-- successfully grabbed a "word" from the document. Returns a new updated
-- selected_text table which should be used (or nil if no expansion could be
-- done).
function LanguageSupport:expandWordSelection(document, selection)
...
function LanguageSupport:addToMainMenu(menu_items)
    ...
    for _, name in ipairs(plugin_names) do
        ...
        if plugin.genMenuItems ~= nil then
            table.insert(sub_table, plugin:genMenuItems())
        else
            table.insert(sub_table, {
                text = plugin.pretty_name or name,
                -- TODO: Figure out a nicer way of displaying a fallback menu.
            })
        end
    end
