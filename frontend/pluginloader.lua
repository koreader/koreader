...
-- TODO: this can be split into smaller functions
...
function PluginLoader:_showIncompatiblePluginsMenu(incompatible_plugins)
    PluginCompatibility.showIncompatiblePluginsMenu(
...
function PluginLoader:genPluginManagerSubItem()
    ...
    for __, plugin in ipairs(self.all_plugins) do
        ...
        if plugin.plugin_ref and plugin.plugin_ref.incompatible then
            ...
            menu_item.sub_item_table = PluginCompatibility.genPluginOverrideSubMenu(plugin.plugin_ref, G_reader_settings)
