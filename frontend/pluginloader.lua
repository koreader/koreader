...
function LoaderSandbox:load_path(plugin, path)
    ...
    package.path  = string.format("%s/?.lua;%s", plugin.path, self.package.path)
    package.cpath = string.format("%s/lib/?.so;%s", plugin.path, self.package.cpath)
    ...
    package.path  = self.package.path
    package.cpath = self.package.cpath
    return ok, re
end
