...
function DictQuickLookup:init()
    ...
    -- Now, dictionaries can be ordered (although yet not per-book), so trust the order set
...
function DictQuickLookup:update()
    ...
    local lookup_word_text = TextWidget:new{
        ...
        face = Font:getFace(word_font_face, word_font_size);
        bold = true,
        max_width = content_width - lookup_edit_button:getSize().w,
    }
    local lookup_word_nb = FrameContainer:new{
        margin = 0,
        bordersize = 0,
        padding = 0,
        padding_right = lookup_edit_button:getSize().w + Size.padding.default,
        overlap_align = "right",
        TextWidget:new{
            text = self.displaynb,
            face = Font:getFace(word_font_face, 22);
            bold = true,
            padding = 0, -- smaller height for better aligmnent with icon
        }
    }
    local lookup_height = math.max(self.word_line_height, lookup_edit_button:getSize().h)
    ...
    if self.ui and self.ui.view.footer_visible then
    ...
    local others_height = 2 * frame_bordersize -- DictQuickLookup border
                        + title_bar:getSize().h
                        + title_sep:getSize().h
    ...
    -- size (we'll then use this perfect height for TextWidget also
    -- for anything that use a HtmlWidget, where it doesn't matter).
...
function DictQuickLookup:changeDictionary(index)
    ...
    if self.is_wiki_fullpage then
        ...
        self.displayword = self.lookupword
        -- self.dictionary = T("%1 / %2  %3", index, #self.results, self.dictionary or "")
