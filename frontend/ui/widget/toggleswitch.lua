    ...
    for i = 1, #self.toggle_content do
        for j = 1, #self.toggle_content[i] do
            if pos == (i - 1) * self.n_pos + j then
                self.toggle_content[i][j].color = self.fgcolor
                self.toggle_content[i][j].background = self.fgcolor
                self.toggle_content[i][j][1][1].fgcolor = Blitbuffer.COLOR_WHITE
            else
                self.toggle_content[i][j].color = self.bgcolor
                self.toggle_content[i][j].background = self.bgcolor
                self.toggle_content[i][j][1][1].fgcolor = Blitbuffer.COLOR_BLACK
            end
        end
    end
end
