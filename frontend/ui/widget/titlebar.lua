...
local TitleBar = OverlapGroup:extend{
    ...
    title_face_fullscreen = Font:getFace("tfont"),
...
function TitleBar:init()
    ...
    if not self.title_top_padding then
        -- We know things are mostly ok without any top padding when using
        -- self.title_face_fullscreen and the other default values, so adjust
        -- the top padding by using it as a reference
        self.title_top_padding = 0
        if self.title_face ~= self.title_face_fullscreen then
            local ref_face_height = self.title_face_fullscreen.ftface:getHeightAndAscender()
            local this_face_height = self.title_face.ftface:getHeightAndAscender()
            self.title_top_padding = math.max(0, math.floor(ref_face_height - this_face_height))
        end
    end
    ...
    local title_h = self.title_widget:getSize().h
    ...
    self.title_group = VerticalGroup:new{
        ...
        self.title_widget,
        self.subtitle_widget and VerticalSpan:new{width = title_subtitle_v_padding},
    ...
    if self.with_bottom_line then
        local filler_and_bottom_line = VerticalGroup:new{
            -- Size.padding.default below the title(s) seems about right with the
            -- default font faces and icon size ratios to have everything balanced.
            VerticalSpan:new{ width = self.titlebar_height + Size.padding.default },
            LineWidget:new{
