...
function BookMapRow:init()
    ...
    for page = self.start_page, self.end_page do
        ...
        if self.bookmarked_pages[page] then
            local has_types = {}
            for _, b in ipairs(self.bookmarked_pages[page]) do
                has_types[b.btype] = true
            end
            local x = self:getPageX(page)
            local w = self:getPageX(page, true) - x
            x = x + math.ceil(w/2)
            ...
            if has_types["highlight"] then
                table.insert(self.indicators, {
                    x = x, y = y,
                    c = 0x2592, -- medium shade
                })
            end
            if has_types["note"] then
                table.insert(self.indicators, {
                    x = x, y = y,
                    c = 0xF040, -- pencil
                    rotation = -90,
...
function BookMapWidget:init()
    ...
    self.vgroup.propagateEvent = function(self, event) return false end
...
function BookMapWidget:update()
    ...
    local row, row_idx, row_y, row_h = self:getMatchingVGroupRow(function(r, r_y, r_h)
...
Below the pages can be found some indicators:
▲ current page
❶ ❷ … previous locations
▒ highlighted text
...
Tap on a location in the book to go browse thumbnails of the pages there.
Swipe along the screen left edge to change the level of chapters to include in the book map, and the type of book map (grid or flat) when crossing the level 0.
Swipe along the screen bottom edge to change the width of page slots.
Swipe or pan vertically on the content to scroll.
Any multiswipe will close the book map.

On a newly opened book, the book map will start in grid mode showing all chapter levels, fitting on a single screen, to give the best initial overview of the book content.]],
    })
end
...
function BookMapWidget:onScrollPageUp()
    ...
    local row, row_idx, row_y, row_h = self:getVGroupRowAtY(-1)
    local to_keep = 0
    if row then
...
function BookMapWidget:onScrollPageDown()
    ...
    local scroll_offset_y = self.cropping_widget._scroll_offset_y
    local row, row_idx, row_y, row_h = self:getVGroupRowAtY(self.crop_height)
    if row then
...
function BookMapWidget:onScrollRowDown()
    local scroll_offset_y = self.cropping_widget._scroll_offset_y
    local row, row_idx, row_y, row_h = self:getVGroupRowAtY(0)
    if row then
...
function BookMapWidget:onSwipe(arg, ges)
    ...
    if (not self._mirroredUI and ges.pos.x < Screen:getWidth() * 1/8) or
        ...
        -- Swipe along the screen left edge: increase/decrease toc levels shown
    ...
    if ges.pos.y > Screen:getHeight() * 7/8 then
        -- Swipe along the screen bottom edge: increase/decrease pages per row
...
function BookMapWidget:onTap(arg, ges)
    ...
    local row, row_idx, row_y, row_h = self:getVGroupRowAtY(y-self.title_bar_h)
