...
function SortWidget:onCancel()
    ...
    if self.orig_item_table then
        -- We can't break the reference to self.item_table, as that's what the callback uses to store the re-ordered data...
        -- So, do this in two passes: empty it, then re-fill it from the copy.
        for i = 0, #self.item_table do
...
function SortWidget:onReturn()
    -- The callback we were passed is usually responsible for passing along the re-ordered table.
    -- And also the enabled flag, meaning we have to honor it even without moves.
