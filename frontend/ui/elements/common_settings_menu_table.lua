...
- all inside koreader/hashdocsettings/ as hashes: sdr folders are identified not by filepath and filename but by partial MD5 hash. This allows you to rename, move, and copy documents outside at will outside of KOReader, and KOReader will still link them to their metadata. However, any file modifications such as writing highlights into PDFs or downloading from Calibre may change the hash, resulting in metadata being unlinked from the file. Calculating file hashes may also impact loading and file browser browsing times. This option may be preferable for users with many documents that want to remove sdr folder clutter or who might have multiple copies of their documents in different directory paths or devices.]])
