...
function ReaderHighlight:onHold(arg, ges)
    ...
    if ok and word then
        ...
        -- highlight to be expanded by LanguageSupport more easily.
        ...
        local new_selected_text = LanguageSupport:expandWordSelection(self.ui.document, self.selected_text)
        ...
        if self.ui.document.info.has_pages then
            ...
            UIManager:setDirty(self.dialog, "ui", Geom:boundingBox(unpack(self.selected_text.sboxes)))
        end
    ...
    elseif selected_text.sboxes and self.hold_pos then
        ...
        if not text then
            ...
            for _, sbox in ipairs(selected_text.sboxes) do
                local word = self.ui.document:getOCRWord(self.hold_pos.page, sbox)
                logger.dbg("OCRed word:", word)
                -- XXX: This will probably produce incorrect results on RTL text.
        ...
        if text and text ~= "" then
            self.ui:handleEvent(Event:new("LookupWord", word, false, word_boxes, self, selected_link))
        else
...
function ReaderHighlight:onHoldRelease()
    ...
    if self.is_word_selection then
        ...
        -- XXX: Not needed, and actually not good -- having this here means
        --      that during DictQuickLookup:init() we have to fetch the text
        --      selection again.
        --self.selected_text = nil
        --self.is_word_selection = false
    else
...
function ReaderHighlight:highlightFromHoldPos()
    if self.hold_pos then
        if not self.selected_text then
            ...
            -- Match text expansion you'd get from self:onHold().
            local new_selected_text = LanguageSupport:expandWordSelection(self.ui.document, self.selected_text)
            if new_selected_text then
                self.selected_text = new_selected_text
            end
