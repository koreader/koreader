...
function ReaderDogear:paintTo(bb, x, y)
    ...
    if self.sides == SIDE_RIGHT or self.sides == SIDE_BOTH then
        self.right_ear:paintTo(bb, x, y)
    end

    -- Exit early if we don't need to paint left side.
    if self.sides ~= SIDE_LEFT and self.sides ~= SIDE_BOTH then
        return
    end
...
function ReaderDogear:getRefreshRegion()
    ...
    -- The dimension of both icons should be the same.
    return self.icon_right.dimen
end

-- @param visible boolean
-- @param side number 1 only left, 2 only right, if 3 both sides, nil == 2
-- TODO(ogkevin): if sides change while we're already showing a dogear, gotta trigger a partial refresh.
-- For some reason, only the left ear has this weird behaviour
function ReaderDogear:onSetDogearVisibility(visible, sides)
    logger.dbg("ReaderDogear:onSetDogearVisibility", visible, sides)
    self.sides = sides or SIDE_RIGHT
