...
function ReaderSearch:onShowSearchDialog(text, direction, search_type, case_insensitive)
    ...
    local has_keys, modifier, direction_left, direction_right
    ...
    local buttons = {
        ...
        },
        {
            text = backward_text,
            vsync = true,
            key_bindings = has_keys and { modifier, direction_left } or nil,
            callback = function()
...
function ReaderSearch:gotoResultsItem(start_index, save_current_location)
    ...
    -- someone already reverses row layout when self.mirrored_ui is true;
    -- this call exists only to correct the delta introduced by invert_buttons
    if invert_buttons then
