...
function ReaderDictionary:init()
    ...
    self.quick_dismiss_before_delay = TimeVal.s2fts(3)
...
function ReaderDictionary:stardictLookup(word, dict_names, fuzzy_search, boxes, link)
    ...
    self._lookup_start_fts = UIManager:getTime_fts()
    local results = self:startSdcv(word, dict_names, fuzzy_search)
    if results and results.lookup_cancelled and (TimeVal.now_fts() - self._lookup_start_tfs) <= self.quick_dismiss_before_delay then
...
function ReaderDictionary:showDict(word, results, boxes, link)
    ...
    if results and results[1] then
        ...
        if not results.lookup_cancelled and self._lookup_start_fts and (TimeVal.now_fts() - self._lookup_start_fts) > self.quick_dismiss_before_delay then
