            ...
            if req.when_generated_callback then -- not cancelled since queued
                ...
                if tile then
                    ...
                    if self:startTileGeneration(req) then
                        ...
                        when_generated_callback(nil, req.batch_id, true)
                    end
                end
            end
        end
...
function ReaderThumbnail:checkTileGeneration(request)
    ...
    if stuff_to_read then
        ...
        if result then
            ...
            when_generated_callback(nil, request.batch_id, true)
        end
...
function ReaderThumbnail:_getPageImage(page)
    ...
    -- Be sure to call the most inner methods enough to get what we want, and
    -- not upper event handlers that may trigger other uneeded events and stuff.
