        ...
        if statm then
            ...
            if rss >= self.memory_threshold then
                if Device:canRestart() then
                    if G_reader_settings:isTrue("device_status_memory_auto_restart") then
...
function ReaderDeviceStatus:addToMainMenu(menu_items)
    ...
    if self.has_battery then
        table.insert(menu_items.device_status_alarm.sub_item_table, 1,
            {
                text = _("Low battery"),
                checked_func = function()
        ...
        table.insert(menu_items.device_status_alarm.sub_item_table, 2,
            {
                text_func = function()
        ...
        table.insert(menu_items.device_status_alarm.sub_item_table, 3,
            {
                text_func = function()
