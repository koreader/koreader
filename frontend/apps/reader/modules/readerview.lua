...
function ReaderView:draw2Pages(bb, x, y)
    ...
    logger.dbg("ReaderView:draw2Pages: page_states", self.page_states)
    ...
    logger.dbg("readerview.draw2pages: got page pairs for state", self.state.page, states)

    -- for i, state in ipairs(states) do
    --     -- local dimen = self.document:getPageDimensions(state.page, 1, 0)
    --     local dimen = self.document:getNativePageDimensions(state.page)
    ...
    --     logger.dbg("readerview.draw2pages: page state dimen", i, state.dimen.w, state.dimen.h)

    --     logger.dbg("readerview.draw2pages: zoom factor", i, zoom)
    ...
    logger.dbg("ReaderView: areas ", x, total_width, visible_area.w, Screen:getWidth())
    -- Calculate positioning
    -- local x_offset = (visible_area.w - total_width) / 2
     -- local x_offset = (x +  total_width) / 2
     -- local x_offset = (x + total_width) / 2
     -- local x_offset = x + (total_width / 2)
     -- local x_offset = 
    local x_offset = x
    -- if total_width < visible_area.w then
    if self.dimen.w > total_width then
        x_offset = x_offset + (self.dimen.w - total_width) /2
         end
     -- local x_offset = self.state.offset.x / 2
     local y_offset = y
    ...
    -- local x_offset = (visible_area.w - states[start_i].dimen.w )/ 2
...
-- If dual page is enabled for cbz, then readerpagging will give us the corret
-- base page in ReaderView:draw2Pages.
