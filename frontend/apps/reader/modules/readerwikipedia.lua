        ...
        callback = function()
            ...
            for i = #wikipedia_history_table, 1, -1 do
                ...
                if value.lang ~= main_lang then
                    -- We show item's lang only when different from first user-set wiki lang
...
function ReaderWikipedia:lookupWikipedia(word, is_sane, box, get_fullpage, forced_lang, dict_close_callback)
    ...
    if forced_lang then
        ...
        -- use last lang selected, or first lang from self.wiki_languages
...
function ReaderWikipedia:getWikiLanguages()
    ...
    -- If for some reason (wiki languages array changed) the last lang is no longer
...
function ReaderWikipedia:onSaveSettings()
    if self.ui.doc_settings and self.wiki_last_language_used then
        ...
        -- means the user is fine with this lang, otherwise he will have changed it;
