...
local shared_state_data = ffi.C.mmap(nil, 3*4, bit.bor(ffi.C.PROT_READ, ffi.C.PROT_WRITE),
...
function ReaderRolling:addToMainMenu(menu_items)
    ...
    menu_items.partial_rerendering = {
        text = _("Enable partial rerenderings"),
        enabled_func = function()
        ...
        hold_callback = function()
            ...
            local text = _([[
With EPUB documents (having multiple fragments), text appearance adjustments can be made quicker by rerendering only the current chapter.
After such partial rerenderings, the book and KOReader are in a degraded state: you can turn pages, but some info and features may be broken or disabled (ie. footer info, ToC, statistics…).
To get back into a sane state, some full rerendering is made in the background, a cache saved, and the book will be reloaded seamlessly when you are idle reading.]])
            if cre_partial_rerendering then
                text = text .. "\n\n" .. _("The current default (★) is to enable partial rerenderings when possible.")
            else
            ...
            UIManager:show(MultiConfirmBox:new{
                text = text,
                -- This text is a bit long, and MultiConfirmBox currenlt doesn't adjust the
...
function ReaderRolling:updatePos(force)
    ...
    if new_rendering_hash ~= self.rendering_hash or force then
        ...
        if self.ui.document:isRerenderingDelayed(true) then
            ...
            logger.dbg("  but rendering delayed, will do partial rerenderings on draw")
...
function ReaderRolling:setupRerenderingAutomation()
    ...
    self._stepRerenderingAutomation = function(next_step)
        ...
        elseif next_step == self.RENDERING_STATE.FULL_RENDERING_IN_BACKGROUND then
            ...
            if self.valid_cache_rendering_hash and
                        self.valid_cache_rendering_hash == self.ui.document:getDocumentRenderingHash(false) then
        ...
        elseif next_step == self.RENDERING_STATE.RELOADING_DOCUMENT then
            ...
            if self.rendering_state == self.RENDERING_STATE.PARTIALLY_RERENDERED then
                ...
                local top_widget = UIManager:getNthTopWidget(1)
                if top_widget and top_widget.name == "ReaderUI" then
                ...
                if self.rendering_state == self.RENDERING_STATE.FULL_RENDERING_READY then
                    ...
                    if not do_reload then
                        ...
                        if self._current_rerendering_pid and not self:_waitOrKillCurrentRerenderingSubprocess(false) then
                            ...
                            until self:_waitOrKillCurrentRerenderingSubprocess(false)
                        end
