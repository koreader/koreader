...
local function copyPageState(page_state)
    return {
        ...
        optimize_scanned = page_state.optimize_scanned,
...
function ReaderPaging:onUpdateScrollPageOptimizeScanned(toggle)
    for _, state in ipairs(self.view.page_states) do
        state.optimize_scanned = toggle
    end
    return true
end
...
function ReaderPaging:getNextPageState(blank_area, image_offset)
    ...
    return {
        ...
        optimize_scanned = self.view.state.optimize_scanned,
        offset = page_offset,
