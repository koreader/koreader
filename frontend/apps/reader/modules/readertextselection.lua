...
function ReaderTextSelection:_getAdjacentWordRolling(word, direction, lock_line_center_y, lock_line_tolerance)
    ...
    -- The Escape Hatch: Loop until we physically exit the origin word's bounding box.
    -- Bounded to 20 iterations to prevent infinite freezes in pathologically nested BiDi spans.
    for _ = 1, 20 do
        ...
        -- Hyphenated words straddled across different pages
        if not doc:isXPointerInCurrentPage(current_xp) then
        ...
        local screen_y, screen_x = doc:getScreenPositionFromXPointer(current_xp)
        if not screen_y or not screen_x then break end

        local try_candidate = self:_getWordFromScreenPoint(screen_x, screen_y, direction, 0)

        -- The Gate: Ensure the geometric hit actually escaped the origin word
        if try_candidate and try_candidate.sbox and try_candidate.pos0 ~= word.pos0 then
            candidate = try_candidate
            break
        else
            logger.dbg("ReaderTextSelection: BiDi box overlap detected, skipping internal node.")
        end
    end
    ...
    if lock_line_center_y and lock_line_tolerance then
        ...
        if math.abs(candidate_center_y - lock_line_center_y) > lock_line_tolerance then
            return nil
        end
    end
