    ...
    battery = function(footer)
        ...
        if Device:hasBattery() then
            ...
            if Device:hasAuxBattery() and powerd:isAuxBatteryConnected() then
                ...
                -- Average both batteries to compute the icon...
                if symbol_type == "icons" or symbol_type == "compact_items" then
                    prefix = powerd:getBatterySymbol(is_charging, math.floor((main_batt_lvl + aux_batt_lvl) / 2))
                end
                -- But sum 'em for the actual text
