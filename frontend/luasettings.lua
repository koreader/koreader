...
-- Handles migration of global Reader settings
function LuaSettings:migrateReaderSettings()
    -- Fine-grained CRe margins (#4945)
    local old_margins = self:readSetting("copt_page_margins")
    if old_margins then
        logger.info("Migrating old global CRe margin settings: L", old_margins[1], "T", old_margins[2], "R", old_margins[3], "B", old_margins[4])
