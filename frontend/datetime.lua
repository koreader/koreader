...
function datetime.secondsToDate(seconds, use_locale)
    ...
    if use_locale then
        ...
        -- @translators 'Date string' use the following placeholders "%1 name of day, %2 name of month, %3 day, %4 year"
...
function datetime.secondsToDateTime(seconds, twelve_hour_clock, use_locale)
    ...
    local message_text = T(C_("%1 date, %2 time", "%1 %2"), BD.wrap(date_string), BD.wrap(time_string))
    return message_text
end
