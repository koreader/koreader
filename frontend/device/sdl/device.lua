...
local function getWindowSizeAndPos()
    local overrides = {
        top = "KOREADER_POS_Y",
        left = "KOREADER_POS_X",
        width = "EMULATE_READER_W",
        height = "EMULATE_READER_H",
    }
    local window = G_reader_settings:readSetting("sdl_window") or {}
    for k, v in pairs(overrides) do
        local override = os.getenv(v)
        if override then
            window[k] = tonumber(override)
        end
    end
    return window
end
